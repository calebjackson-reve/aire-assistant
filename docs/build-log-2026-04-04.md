...
## Data Layer Merge — COMPLETE
**Executed:** 2026-04-04
**Agent:** Agent 1 (Infrastructure & Data Merge)
**Source:** aire-intelligence codebase merged into aire-assistant

### What Was Merged (19 files created)

**Scoring Engines (6 files) → `lib/data/engines/`:**
- `ensemble.ts` — Weighted AVM (MLS 40%, PropStream 25%, Zillow 20%, Redfin 15%)
- `pps.ts` — Pricing Position Score (0-100, 6 weighted factors)
- `bps.ts` — Buyer Perception Score (0-100, 7 admin-reviewed sub-scores)
- `uri.ts` — Upgrade ROI Index (11 BR-calibrated upgrade templates)
- `disagreement.ts` — Source confidence tiers (HIGH/MEDIUM/LOW)
- `normalize.ts` — Address normalization + property_id generation + field mapping

**Database Query Layer (3 files) → `lib/data/db/`:**
- `client.ts` — Neon serverless Pool for raw SQL queries (intelligence tables)
- `queries/properties.ts` — properties_clean + market_snapshots CRUD
- `queries/scores.ts` — aire_scores append-only writes + reads

**Static Data (1 file):**
- `market-data.ts` — BR metro stats, 8 neighborhood heat scores, commercial, rentals

**Barrel Export:**
- `lib/data/index.ts` — unified import point for all engines + queries

**API Routes (5 files) → `app/api/data/`:**
- `POST /api/data/estimate` — Calculate AIRE Estimate with ensemble + confidence
- `GET /api/data/property?id=X` or `?address=X` — Property lookup with snapshot + score
- `GET /api/data/market` or `?neighborhood=zachary` — Market data
- `GET/POST /api/data/scores?property_id=X` — Score retrieval + PPS calculation
- `GET /api/data/health` — DB connectivity + data freshness

### Architecture Decisions
- **Dual DB layer:** Prisma for app models (User, Transaction, etc.) + raw SQL via `@neondatabase/serverless` Pool for intelligence tables (properties_clean, market_snapshots, aire_scores)
- **No Prisma models for intelligence tables** — they're managed by MCP ingestion servers and don't need ORM
- **Pure function engines** — all scoring engines have zero DB dependency, can be used anywhere
- **Build verified:** `npm run build` passes with 0 errors, all 5 API routes compiled

### How Agents Use This
- **Morning Brief** can now call `calculateEnsemble()` for deal valuations
- **Voice Command** can use `findByPropertyId()` for property lookups
- **Intelligence Agent** has full PPS/BPS/URI scoring
- **All agents** can access `AIRE_DATA` for market context
