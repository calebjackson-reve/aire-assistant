...
interface DeadlineItem { name: string; dueDate: string; transactionAddress: string; urgency: string }
interface PipelineItem { address: string; status: string; price: number; daysToClose: number | null; needsAttention: boolean; reason?: string }
      ...
      {!brief ? (
        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between bg-[#f5f2ea] rounded-lg px-4 py-3">
            ...
            <span className="font-mono text-[10px] text-[#6b7d52]/50">
              Generated 06:30 CT
            </span>
          </div>
              ...
              <div className="mt-3 border border-[#e8e4d8] rounded-lg overflow-hidden hidden sm:block">
                ...
                {deadlines.map((d, i) => (
                  <div key={i} className={`grid grid-cols-[auto_1fr_1fr_auto] gap-x-4 px-4 py-2.5 items-center ${
                    ...
                    <span className="font-mono text-xs text-[#1e2416]/60">
              ...
              <div className="mt-3 space-y-2 sm:hidden">
                {deadlines.map((d, i) => (
                  <div key={i} className="bg-[#f5f2ea]/60 rounded-lg px-4 py-3 flex items-start gap-3">
                    <span className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${urgencyDot[d.urgency] || urgencyDot.upcoming}`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-[#1e2416] text-sm">{d.name}</p>
                      <p className="text-[#6b7d52]/60 text-xs truncate">{d.transactionAddress}</p>
                    </div>
                    <span className="font-mono text-xs text-[#1e2416]/60 shrink-0">
                      {new Date(d.dueDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                    </span>
                  </div>
                ))}
          ...
          {/* 05 — Action items */}
          {brief.actionItems && Array.isArray(brief.actionItems) && (brief.actionItems as unknown as ActionItem[]).length > 0 && (
            <section>
              <SectionHeader number="05" title="Required Actions" count={(brief.actionItems as unknown as ActionItem[]).length} />
              <div className="mt-3 space-y-1">
                {(brief.actionItems as unknown as ActionItem[]).map((item, i) => (
                  <div key={i} className="flex items-start gap-3 px-4 py-2.5">
                    <span className="font-mono text-[11px] text-[#9aab7e]/60 mt-0.5 w-4 shrink-0">
                      {i + 1}.
                    </span>
